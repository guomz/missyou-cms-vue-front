...
class Coupon {
  static types = ['满减券', '无门槛折扣券', '无门槛减除券', '满金额折扣券']
  static wholeStoreTypes = ['非全场','全场']
  ...
  static async getCouponTemplates() {
    const res = await get('v1/coupon/template/list')
    return res
  }
  ...
  static async getCouponCategories(id){
    const res = await get(`v1/coupon/categories`, {
      id
    })
    return res
  }
