      ...
      <el-row>
        <el-col :lg="16" :md="20" :sm="24" :xs="24">
          <el-form :model="form" status-icon ref="form" label-width="100px" @submit.native.prevent>
            ...
            <el-form-item label="是否全场通用" prop="whole_store">
              <el-select :change="wholeStoreChange(form.whole_store)" v-model="form.whole_store" placeholder="请选择是否为全场券">
                <el-option v-for="(item, index) in wholeStoreTypes" :key="item" :label="item" :value="index"></el-option>
              </el-select>
            </el-form-item>

            <el-form-item v-if="!isWholeStore" label="选择分类" prop="categoryList">
                <el-cascader placeholder="可多选" v-model="categoryIds" :props="cascaderProps"></el-cascader>
            </el-form-item>
            ...
            <el-form-item class="submit">
              <el-button
                v-permission="{ permission: ['创建Coupon', '更新Coupon'], type: 'disabled' }"
                type="primary"
...
export default {
    ...
    return {
      ...
      cascaderProps:{
        ...
        async lazyLoad(node, resolve) {
          const { level } = node
          try {
            const allSubCategories = await Category.getList()
            const nodes = allSubCategories.map(item => ({
              value: item.id,
  ...
  async mounted() {
    ...
    this.wholeStoreTypes = Coupon.wholeStoreTypes
    if (!this.isCreate) {
      ...
      //获取当前优惠券的分类
      const categories = await Coupon.getCouponCategories(this.couponId)
  ...
  methods: {
    async submitForm(formName) {
      ...
      try {
        ...
        postData.categories = this.categoryIds.map(categoryId => categoryId[0])
    ...
    //选择是否全场券选项事件
    wholeStoreChange(value){
      // console.log(value)
      if(value == 1){
        this.isWholeStore = true
      }else{
        this.isWholeStore = false
      }
    },
